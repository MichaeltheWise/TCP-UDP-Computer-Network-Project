...
int main ()
{
	setup_UDP_socket();
	bind_UDP_socket();
	cout << "The server C is up and running using UDP on port 23823" << endl;
	int length;
	int* buf;
	char* instr;
	int answer;
	while (1)
	{
		...
		cout << "The server C has received " << length / 3 << " numbers" << endl;
		// cout << length << endl;
		...
		// cout << instr << endl;
		answer = calculationC(instr, length, buf);
		cout << "The server C has successfully finished the reduction " << instr << ": " << answer << endl;
		// cout << answer << endl;
...
void bind_UDP_socket()
{
	...
	UDP_server_socket_addr.sin_port = htons(23823);
...
int calculationC(char* instr, int length, int* data) 
{
	int actual_size = length / 3;
	string s_instr = instr;
	int min = data[2*actual_size];
	int max = data[2*actual_size];
	int sum = 0;
	int sos = 0;
	if (s_instr == "MIN")
	{
		for (int i = (2*actual_size) + 1; i < length; i++)
		{
			if (data[i] < min)
			{
				min = data[i];
	...
	{
		for (int i = 2*actual_size; i < length; i++)
		{
			sum = sum + data[i];
		}
		return sum;
