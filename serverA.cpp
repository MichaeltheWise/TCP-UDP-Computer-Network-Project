...
int main ()
{
	setup_UDP_socket();
	bind_UDP_socket();
	cout << "The server A is up and running using UDP on port 21823" << endl;
	int length;
	int* buf;
	char* instr;
	int answer;
	while (1)
	{
		...
		cout << "The server A has received " << length / 3 << " numbers" << endl;
		// cout << length << endl;
		...
		// cout << instr << endl;
		answer = calculationA(instr, length, buf);
		cout << "The server A has successfully finished the reduction " << instr << ": " << answer << endl;
		// cout << answer << endl;
...
void bind_UDP_socket()
{
	...
	UDP_server_socket_addr.sin_port = htons(21823);
...
int calculationA(char* instr, int length, int* data) 
{
